...
/**
 * Foundation: Frontdesk — RAG (Embeddings) engine
 ...
 * Build: 2025-08-20
 */
class Foundation_Frontdesk_RAG {
    const TABLE = 'fnd_frontdesk_chunks';

    public static function init() {
        add_action('plugins_loaded', [__CLASS__, 'maybe_create_table']);

        // AJAX: Start/Status/Stop
        add_action('wp_ajax_fnd_frontdesk_rag_start',  [__CLASS__, 'ajax_start']);
        add_action('wp_ajax_fnd_frontdesk_rag_status', [__CLASS__, 'ajax_status']);
        add_action('wp_ajax_fnd_frontdesk_rag_stop',   [__CLASS__, 'ajax_stop']);
    ...
    private static function maybe_add_fulltext_index($table) {
        ...
        if ($result === false && defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Frontdesk] FULLTEXT index could not be added (engine/version may not support it).');
        }
    }
    ...
    private static function embed_array($strings) {
        ...
        $resp = wp_remote_post('https://api.openai.com/v1/embeddings', [
            'headers' => [
                ...
                'User-Agent'    => 'Foundation-Frontdesk/1.0 (+wp)',
            ],
    ...
    public static function ajax_start() {
        check_ajax_referer('fnd_frontdesk_admin', 'nonce');
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => __('Unauthorized', 'foundation-frontdesk')], 403);
        }
    ...
    public static function ajax_stop() {
        ...
        $st = get_option('fnd_frontdesk_rag_status', []);
        $st['status'] = 'stopped';
        update_option('fnd_frontdesk_rag_status', $st, false);
    ...
    public static function retrieve_context($query, $k = 6) {
        ...
        $synonyms = apply_filters('fnd_frontdesk_query_synonyms', [], $query);
        ...
        if ($has_fulltext && $search_terms) {
            $prefilter_limit = (int) apply_filters('fnd_frontdesk_prefilter_limit', 500);
        ...
        foreach ($top as $t) {
            ...
            $content = sanitize_textarea_field((string) $r->content);
            $type = sanitize_key((string) ($r->post_type ?? 'page'));
